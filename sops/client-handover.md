...
| Owner | Action | When |
|---|---|---|
| Outgoing GTME | Complete the Handover Brief (using `gtm-skills/handover-brief-writer.md`) | Before handover date, immediately if unplanned |
| Aaron | Review brief and assign incoming GTME | Within 24 hours of brief submission |
| Outgoing GTME | Loom walkthrough for complex accounts | Before last day or before leave starts |
| Incoming GTME | Review brief and raise questions | Before taking ownership |
| CSM | Send client introduction message (Scenarios 1 and 2) | On or before handover date (GTME leaving) |
| Outgoing GTME | Send client introduction message (Scenario 3 only) | On or before handover date (internal reassignment only) |
...
with `gtm-skills/handover-brief-writer.md` (pending, this SOP is a forward reference ahead of that skill's
arrival).

- **Client:** name, plan (SalesOS / PerformanceOS), MRR.
...
## Scenario 1 - GTME Leaving (Permanent and Phased)
...
5. CSM sends the client introduction message (see Client Communication below).
6. Incoming GTME takes full ownership, transition complete.

---

## Scenario 2 - GTME Leaving (Permanent and Immediate)

Timeline: brief must be completed before the GTME's last day.

1. CSM completes the Handover Brief for every active client.
2. CSM/Aaron reviews all briefs and assigns incoming GTMEs.
3. Incoming GTME reviews the brief and raises any questions before taking over.
4. CSM sends the client introduction message (see Client Communication below).
5. Incoming GTME takes full ownership, transition complete.

---

## Scenario 3 - Client Reassigned Internally
...
4. Outgoing GTME sends the client introduction message (see Client Communication below).
5. Incoming GTME takes ownership from the agreed date.

---

## Scenario 4 - GTME on Leave (Temporary Cover)
...
5. No client introduction needed unless the absence is 2+ weeks.
6. If the absence is 2+ weeks, see Client Communication - Scenario 4 below.

---

## Client Communication - Scenarios 1 and 2 - Permanent Departure
...
## Client Communication - Scenario 3 - Internal Reassignment

Outgoing GTME message:
...
## Client Communication - Scenario 4 - Temporary Cover (2 weeks or longer only)

Assigned GTME message, if planned:
...
- **Aaron is the same escalation contact named in `sops/gtme-escalation-to-am.md` and
  ...
  but the same person.
- **The Handover Brief is generated by `gtm-skills/handover-brief-writer.md`**, which has not been added to
  the OS yet, this is a forward reference pending that skill.
- **Scenario 4's "no new strategies without checking AM first"** is a tighter version of the caution already
  in `wiki/_active-test-rules.md` and `gtm-skills/campaign-optimiser.md`'s "one variable at a time" rule, a
  covering GTME should be even more conservative than a permanent owner would be.
