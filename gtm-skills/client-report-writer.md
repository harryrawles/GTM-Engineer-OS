...
- If a campaign is Red (PRR under 0.2%, per `sops/campaign-performance-standards.md`), name the root cause and the fix - do not hide it. This should never be the client's first time hearing it, `sops/client-not-getting-results.md` requires flagging it before the report does.
