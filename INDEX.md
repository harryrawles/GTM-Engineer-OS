...
| sops/ | Standard operating procedures. `sops/instantly-api.md` = how the OS calls each client's Instantly workspace via the API wrapper `.claude/bin/instantly.sh` (per-client keys, read/write gating). `sops/daily-structure.md` = default daily block structure + response-time defaults. `sops/client-communication-principles.md` = baseline comms standard + situational guidance (cancellations, refunds, rush demands, client-written copy). `sops/am-gtme-responsibility-split.md` = who owns what between GTME and AM. Add more runbooks over time (`sops/README.md`). |
