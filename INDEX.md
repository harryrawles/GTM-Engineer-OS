...
| sops/ | Standard operating procedures. `sops/instantly-api.md` = how the OS calls each client's Instantly workspace via the API wrapper `.claude/bin/instantly.sh` (per-client keys, read/write gating). `sops/daily-structure.md` = default daily block structure + response-time defaults. `sops/client-communication-principles.md` = baseline comms standard + situational guidance (cancellations, refunds, rush demands, client-written copy). `sops/am-gtme-responsibility-split.md` = who owns what between GTME and AM. `sops/campaign-performance-standards.md` = authoritative PRR/sends-per-reply benchmark tiers and what stays internal vs. what's told to clients. `sops/gtme-unlock-ladder.md` = the escalation ladder when stuck on a client (Cowork, support channel, `#grill-my-campaigns`). `sops/client-not-getting-results.md` = proactive-disclosure protocol for underperforming results. `sops/tough-client-involve-csm.md` = when to flag the CSM on a difficult or disengaged client. `sops/gtme-escalation-to-am.md` = AM escalation triggers, message format, and tagging matrix. `sops/client-handover.md` = transitioning a client to a new or covering GTME. `sops/solving-campaign-issues.md` = tactical fix playbook for underperforming campaigns (CTA plays, deliverability, ICP-miss recovery). `sops/churn-prevention.md` = onboarding-time churn prevention and the cancellation-save discount boundary. `sops/onboarding-documentation.md` = client-facing onboarding message cadence and call checklists (standard flow). `sops/onboarding-24-hour.md` = accelerated AI-SDR-first onboarding flow for the 24-Hour Onboarding upgrade. Add more runbooks over time (`sops/README.md`). |
...
| best-practices/growth-plays.md | Client-agnostic tactical plays (re-run non-repliers, add a sequence step) for extracting more value from an existing list without fresh data, used by `gtm-skills/client-checkin-writer.md` |
...
| gtm-skills/workspace-deep-report.md | On-demand deep-dive across a client's whole Instantly workspace (GTME picks the client), catches risk early, drafts a tone-matched client message, and, on confirmation, logs the finding to decision-log.md/campaign-state.md for cold-email-writer.md to draw on |
| gtm-skills/client-checkin-writer.md | Three-times-weekly (Mon plan / Wed results / Fri recap + next week), portfolio-wide relationship check-in, warm and never performance-graded, distinct from the performance-driven skills above, logs every send to comms-log.md |
